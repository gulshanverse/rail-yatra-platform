    ...
    <section className="space-y-5" aria-label="Journey decision workspace">
      <header className="rounded-3xl border border-white/10 bg-white/[0.035] p-5 sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold tracking-tight text-white sm:text-3xl">{data.origin || 'Your journey'} <span className="mx-1 text-slate-500">→</span> {data.destination || 'Destination'}</h2>
          ...
          <DecisionBadge label={verificationCopy[verification]} tone={verificationTone[verification]} />
        </div>
      ...
      {recommended ? (
        <article className="overflow-hidden rounded-3xl border border-blue-400/20 bg-gradient-to-br from-blue-500/[0.14] via-white/[0.035] to-violet-500/[0.10] p-5 shadow-xl shadow-blue-950/20 sm:p-7">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-blue-300/15 bg-blue-300/10 px-3 py-1.5 text-xs font-semibold text-blue-100">
                <Sparkles className="h-3.5 w-3.5" aria-hidden="true" />
                Best overall
              </div>
              <p className="mt-5 text-sm font-medium text-slate-400">{recommended.trainNumber ? `Train ${recommended.trainNumber}` : 'Recommended journey'}</p>
              <h3 className="mt-1 text-2xl font-bold tracking-tight text-white sm:text-3xl">{recommended.trainName || 'Best available option'}</h3>
            </div>
            <div className="flex flex-wrap items-center gap-2 sm:justify-end">
              {confidenceValue !== undefined ? <DecisionBadge label={formatConfidence(confidenceValue) ?? 'Confidence unavailable'} tone="ai" /> : null}
              <DecisionBadge label={verificationCopy[verification]} tone={verificationTone[verification]} />
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs">
                {recommended.className ? <DecisionBadge label={recommended.className} /> : null}
                {recommended.changes !== undefined ? <DecisionBadge label={recommended.changes === 0 ? 'Direct' : `${recommended.changes} change${recommended.changes > 1 ? 's' : ''}`} tone={recommended.changes === 0 ? 'positive' : 'neutral'} /> : null}
                {recommended.availability ? <DecisionBadge label={recommended.availability} tone={availabilityTone(recommended.availability)} /> : null}
                {recommended.risk ? <DecisionBadge label={`${recommended.risk[0].toUpperCase()}${recommended.risk.slice(1)} risk`} tone={recommended.risk === 'low' ? 'positive' : recommended.risk === 'high' ? 'warning' : 'neutral'} /> : null}
              </div>
            </div>
            <div className="lg:text-right">
              {recommended.fare ? <p className="text-2xl font-bold text-white">{recommended.fare}</p> : <p className="text-sm font-medium text-slate-500">Fare not available</p>}
              <button type="button" onClick={() => onSelectOption?.(recommended)} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:-translate-y-0.5 hover:bg-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300/70">
                View journey <ArrowRight className="h-4 w-4" aria-hidden="true" />
              </button>
            </div>
          </div>
        ...
        <article className="rounded-3xl border border-blue-400/15 bg-gradient-to-br from-blue-500/[0.09] via-white/[0.03] to-violet-500/[0.06] p-5 sm:p-7">
          <div className="flex items-center gap-2 text-sm font-semibold text-blue-200"><Sparkles className="h-4 w-4" aria-hidden="true" /> AI analysis received</div>
          <p className="mt-3 text-sm leading-6 text-slate-300">RailYatra has analyzed this journey. Structured train options will appear here when live journey data is available.</p>
          {data.analysis ? <p className="mt-4 line-clamp-4 rounded-2xl border border-white/8 bg-black/10 p-4 text-sm leading-6 text-slate-400">{data.analysis}</p> : null}
        </article>
      )}
