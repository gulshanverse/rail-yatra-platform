  ...
  static async requestWithRetry<T>(
    ...
    if (this.state === 'OPEN') {
      if (now >= this.nextAttemptTime) {
        ...
        this.logger.warn('Circuit Breaker is OPEN. Blocking request and returning fallback.');
        throw new Error('Circuit Breaker is open. Downstream service is currently unreachable.');
      }
    }
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      try {
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Request Timeout Limit Breached.')), timeoutMs)
        );
        ...
        if (this.state !== 'CLOSED') {
          this.logger.log('Circuit Breaker recovered. Resetting status to CLOSED.');
        }
        this.state = 'CLOSED';
        ...
        lastError = err;
        this.logger.warn(`Resilience request failed (Attempt ${attempt}/${maxRetries + 1}): ${err.message}`);
    ...
    if (this.consecutiveFailures >= failureThreshold) {
      ...
      this.logger.error(`Circuit Breaker tripped to OPEN. Muting requests for ${resetTimeoutMs}ms`);
    }
