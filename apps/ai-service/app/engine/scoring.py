...
class ScoringEngine:
    ...
    def compute_sub_scores(
        self, option: Dict[str, Any], distance_km: int = 500
    ) -> Dict[str, float]:
        """
        ...
        cost_score = round(
            100.0 * (1.0 - (fare_ratio * 0.8)), 1
        )  # offset so even expensive trains don't score 0
        ...
        speed_score = round(
            min(100.0, max(10.0, (avg_speed - 40.0) * (90.0 / 50.0))), 1
        )
        ...
        reliability_score = round(
            max(0.0, (conf_prob * 0.7) + (30.0 - delay_penalty)), 1
        )
    ...
    def determine_reason_codes(
        self, sub_scores: Dict[str, float], option: Dict[str, Any]
    ) -> List[str]:
        """
        ...
        # PNR Specific checks
        if (
            option["confirmation_probability"] >= 85
            and "WL" in option["waitlist_status"]
        ):
    ...
    def calculate_overall_score(
        self, sub_scores: Dict[str, float], preferences: UserPreferences
    ) -> int:
        """
