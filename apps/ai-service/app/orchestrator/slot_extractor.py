...
class SlotExtractor:
    ...
    def extract_slots(
        ...
        if train_match:
            ...
            slots["train_number"] = Slot(
                name="train_number", value=val, type="TrainNumber", confidence=1.0
            )
        ...
        if date_match:
            ...
            if "today" in lower_text:
                slots["date"] = Slot(
                    name="date", value="today", type="RelativeDate", confidence=0.9
                )
            elif "tomorrow" in lower_text:
                slots["date"] = Slot(
                    name="date", value="tomorrow", type="RelativeDate", confidence=0.9
                )
        ...
        passenger_match = re.search(
            r"\b(\d+)\s*(?:passenger|adult|people|traveler|seat)s?\b", lower_text
        )
        if passenger_match:
            val = int(passenger_match.group(1))
            slots["passenger_count"] = Slot(
                name="passenger_count", value=val, type="PassengerCount", confidence=0.9
            )
