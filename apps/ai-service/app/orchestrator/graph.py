...
def get_compiled_graph():
    ...
    if _compiled_graph is None:
        ...
        if not agent_keys:
            ...
            agent_keys = ["travel_decision", "prediction", "pnr", "knowledge", "conversation"]
        ...
        def route_decision(state: AIState) -> str:
            if state.get("errors") and not state.get("response"):
                logger.warning(f"Errors detected during state processing. Routing to {NODE_ERROR}")
                return NODE_ERROR
                
            selected = state.get("selected_agent")
            if selected in agent_keys:
                return selected
            logger.warning(f"Selected agent '{selected}' not found. Defaulting to conversation.")
