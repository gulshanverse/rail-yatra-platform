...
class MockChatModel(BaseChatModel):
    ...
    def _generate_mock_response(self, messages: List[BaseMessage]) -> str:
        ...
        if (
            "target intents" in system_prompt
            or "intent classification" in system_prompt
        ):
            intent = "conversation"
            ...
            if any(
                ...
                k in user_query for k in ["pnr", "ticket status", "booking status"]
            ):
                intent = "check_pnr"
                reason = "Inquiry regarding checking a ticket PNR status"
            elif any(
                k in user_query for k in ["policy", "luggage", "refund", "faq", "rules"]
            ):
                intent = "knowledge"
                reason = "General query about railway procedures or guidelines"
            elif any(
                k in user_query
                for k in ["waitlist", "delay", "confirm", "forecast", "prediction"]
            ):
                intent = "journey_intelligence"
                reason = "Predictive intelligence check"

            return json.dumps({"intent": intent, "confidence": 0.96, "reason": reason})

        # Standard agent mock responses
        if (
            "specialized travel decision agent" in system_prompt
            or "travel_decision" in system_prompt
            or "plan_travel" in system_prompt
        ):
            return (
        ...
        if (
            "smart recommendation agent" in system_prompt
            or "recommendation" in system_prompt
        ):
            return (
        ...
        if (
            "ticket confirmation prediction agent" in system_prompt
            or "prediction" in system_prompt
        ):
            return (
