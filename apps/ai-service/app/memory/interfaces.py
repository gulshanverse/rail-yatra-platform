...
class MemoryItem(BaseModel):
    ...
    vector: Optional[List[float]] = Field(
        default=None, description="Optional dense embedding vector"
    )
    ...
    parent_version_id: Optional[str] = Field(
        default=None, description="Parent version state UUID"
    )
    schema_version: int = Field(
        default=1, description="Data schema version for evolutionary mapping"
    )
    ...
    access_count: int = Field(
        default=0, description="Read counter for frequency analytics"
    )
    importance: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Importance score (0.0 to 1.0)"
    )
    confidence: float = Field(
        default=1.0,
        ge=0.0,
        le=1.0,
        description="Cognitive confidence score (0.0 to 1.0)",
    )
    is_active: bool = Field(
        default=True, description="Soft deletion/active state toggle"
    )
...
class ConversationSession(BaseModel):
    ...
    history: List[Dict[str, Any]] = Field(
        default_factory=list, description="Sliding window chat history messages"
    )
    context: Dict[str, Any] = Field(
        default_factory=dict, description="Transient session parameters"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="System metadata parameters"
    )
...
class IMemoryRanker(Protocol):
    ...
    def score_memories(
        self, items: List[MemoryItem], query: str, config: Dict[str, Any]
    ) -> List[MemoryItem]:
