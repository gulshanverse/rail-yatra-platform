...
class LongTermMemory:
        ...
        try:
            async with httpx.AsyncClient(timeout=3.0) as client:
                response = await client.get(
                    f"{self.backend_url}/api/users/{user_id}/preferences"
                )
                if response.status_code == 200:
                    return response.json()
        except Exception as e:
            logger.debug(
                f"Failed to fetch real preferences from NestJS: {e}. Using fallback."
            )
    ...
    async def get_travel_history(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=3.0) as client:
                response = await client.get(
                    f"{self.backend_url}/api/users/{user_id}/trips"
                )
                if response.status_code == 200:
                    return response.json()
        except Exception as e:
    ...
    async def get_pnr_history(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=3.0) as client:
                response = await client.get(
                    f"{self.backend_url}/api/users/{user_id}/pnrs"
                )
                if response.status_code == 200:
                    return response.json()
        except Exception as e:
