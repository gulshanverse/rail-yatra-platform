...
class BackgroundRailwaySyncer:
    ...
    async def start(self) -> None:
        ...
        if self.running:
            return
        self.running = True
        logger.info(f"Starting Background Railway Syncer loop (Interval: {settings.SYNC_INTERVAL_SECS} seconds)...")
            ...
            try:
                ...
                for t_num in target_trains:
                    ...
                    if delay_info:
                        ...
                        railway_cache_manager.set("delay", t_num, delay_info.model_dump())
                        
                    # Sync schedules / details
                    trains = await self.provider.search_trains("NDLS", "BPL") # Ndls-Bpl sector schedules
                    for t in trains:
                        railway_cache_manager.set("schedule", t.train_number, t.model_dump())
