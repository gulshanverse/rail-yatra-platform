...
class TestAIServiceCore(unittest.TestCase):
    ...
    def test_short_term_memory(self):
        ...
        short_term_memory.save_session_context(session_id, context)
        loaded_context = short_term_memory.get_session_context(session_id)
        ...
        short_term_memory.add_message(session_id, "user", "Hello AI")
        short_term_memory.add_message(session_id, "assistant", "Hello Passenger")
    ...
    def test_intent_classification(self):
        ...
        async def run_classification():
            ...
            res_travel = await intent_classifier.classify("I want to travel from NDLS to BPL next Monday")
            self.assertEqual(res_travel["intent"], "plan_travel")

            # Query mapped to check_pnr
            res_pnr = await intent_classifier.classify("Check status for PNR 4210987654")
            self.assertEqual(res_pnr["intent"], "check_pnr")

            # Query mapped to journey_intelligence
            res_intel = await intent_classifier.classify("what is the confirmation probability of waitlist WL 14?")
