...
class TestJourneyIntelligenceEngine(unittest.TestCase):
    ...
    def test_prediction_certainty_mapping(self):
        ...
        res_cnf = loop.run_until_complete(
            predict_journey_metrics("12002", "3A", waitlist_position=None)
        )
        ...
        res_wl = loop.run_until_complete(
            predict_journey_metrics("12002", "3A", waitlist_position=12)
        )
    ...
    def test_full_engine_optimization_pipeline(self):
        ...
        async def run_pipeline():
            ...
            self.assertEqual(
                len(recommendation.options),
                len(
                    set(
                        [
                            o.train_number + o.source + o.journey_date
            ...
            self.assertTrue(
                recommendation.options[0].overall_score
                >= recommendation.options[-1].overall_score
            )
            self.assertTrue(len(recommendation.tradeoffs_summary) > 0)

            # Check reason codes
            self.assertTrue(
                any(len(opt.reason_codes) > 0 for opt in recommendation.options)
            )
