...
def test_parse_sse_preserves_incomplete_network_chunk():
    complete_event = format_sse({"type": "token", "value": "hello"})
    split_at = complete_event.index('"hello"') + 4
    first = complete_event[:split_at]
    second = complete_event[split_at:]
    ...
    assert events == [
        SSEEvent(data=json.dumps({"type": "token", "value": "hello"}))
    ]
    assert remainder == ""
...
def test_format_sse_is_json_safe_and_terminated():
    rendered = format_sse(
        {"type": "token", "value": "नमस्ते"}, event="message", event_id="7"
    )
