...
from app.memory.healing import (
    circuit_breaker,
    FailureDetectionEngine,
    SelfHealingController,
)
from app.memory.recovery import (
    RecoveryPriorityQueue,
    RecoveryAuditTrail,
    MemorySynchronizer,
    RecoveryManager,
)
from app.memory.workers import (
    supervisor,
    TTLCleanupWorker,
    LockCleanupWorker,
    BaseWorker,
...
async def test_health_routes_endpoints():
    ...
    short_term_memory.fallback_store.save_session(
        "user-1", "sess-active", "{}", '{"memory_version": 4}'
    )
    ...
    with patch.object(
        monitor,
        "evaluate_status",
        return_value={"status": "UNAVAILABLE", "dependencies": {}},
    ):
        res = client.get("/ready")
